...
import { X, Upload, MapPin, Camera, CheckCircle2, Loader2 } from 'lucide-react';
...
export default function SubmitIntelModal({ location, onClose, onSuccess }: SubmitIntelProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!user) {
        alert("Authentication required to submit property.");
        setUploading(false);
        return;
      }
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          ...
          status: 'active',
          source: 'web_agent',
          cover_image_url: fileUrls[0] || null,
          details: {
             lazy_files: fileUrls 
          }
        });
      ...
      >
        ...
        <div className="p-4 border-b border-white/10 flex justify-between items-center bg-white/5">
          <div>
            ...
            <p className="text-[10px] text-gray-500 font-mono">
              GPS: {location.lat.toFixed(5)}, {location.long.toFixed(5)}
            </p>
          </div>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar space-y-6">
           ...
           <div className="grid grid-cols-2 gap-4">
              <div>
                 <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 block">Listing Type</label>
              ...
              <div>
                 <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 block">Price / Valuation</label>
           ...
           {/* LOCATION */}
           <div>
              <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 block">Property Location (Landmark/Area)</label>
              <div className="relative">
           ...
           {/* DESCRIPTION */}
           <div>
              <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 block">Property Description</label>
              <textarea 
           ...
           {/* FILE UPLOAD */}
           <div>
              <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 block">Property Photos</label>
              <div 
              ...
              >
                 <Upload size={24} className="text-gray-500 group-hover:text-emerald-400 mb-2 transition-colors" />
              ...
              {previews.length > 0 && (
                 <div className="flex gap-2 mt-4 overflow-x-auto pb-2 scrollbar-hide">
                    {previews.map((src, i) => (
                       <div key={i} className="w-16 h-16 rounded border border-white/10 flex-shrink-0 overflow-hidden relative group">
        ...
        <div className="p-4 border-t border-white/10 bg-black/40">
           ...
           >
             {uploading ? (
               ...
               <>Publish Listing <CheckCircle2 size={16} /></>
             )}
           </button>
        </div>
