...
export default function AstaMap() {
  ...
  useEffect(() => {
    if (urlListingId && listings.length > 0) {
      ...
      if (prop) {
        ...
        if (mapRef.current) {
          mapRef.current.flyTo({ center: [prop.long, prop.lat], zoom: 16, duration: 2000 });
        }
      }
    }
  ...
  useEffect(() => {
    fetch(`${ENGINE_URL}/api/trends`).then((res) => res.json()).then((data) => {
  ...
  const submitFeedback = async () => {
    ...
    try {
      ...
      if (!error) {
        setFeedbackText(""); setShowFeedback(false); alert("Bug Hunter: System anomaly logged. Command notified.");
      } else throw error;
    } catch (err) {
  ...
  const selectSuggestion = async (item: Suggestion) => {
    ...
    if (item.type === 'feature') {
      const matches = listings.filter((l) => l.vibe_features.toLowerCase().includes(item.value.toLowerCase()));
      if (matches.length > 0) {
    ...
    if (item.placeId) {
      const details = await getPlaceDetails(item.placeId); handlePlaceResult(details, item.value); return;
    }
  ...
  const handlePlaceResult = (place: any, queryName: string) => {
    if (place) {
      if (place.viewport) {
        ...
        const hasListings = listings.some((l) => l.lat >= southwest.lat && l.lat <= northeast.lat && l.long >= southwest.lng && l.long <= northeast.lng);
        if (!hasListings) setTimeout(() => setShowEmptyState({ location: place.name || queryName, type: "hunter" }), 1500);
      } else mapRef.current?.flyTo({ center: [place.location.lng, place.location.lat], zoom: 14, duration: 2500 });
    } else alert("Asta Intelligence could not locate " + queryName);
    setIsSearching(false);
  };

  const executeSearch = async (query: string) => {
    const q = query.trim(); if (!q) return; selectSuggestion({ type: 'location', value: q });
  };
  ...
  const handleInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const localMatches = Array.from(new Set(listings.map((l) => l.location_name))).filter((l) => l.toLowerCase().includes(lower)).slice(0, 2).map((l) => ({ type: "location", value: l } as Suggestion));
  ...
  useEffect(() => {
    if (googlePredictions && googlePredictions.length > 0) {
      const googleSuggestions = googlePredictions.filter((p: any) => p.description.toLowerCase().includes("ghana")).slice(0, 3).map((p: any) => ({ type: "location", value: p.description, placeId: p.place_id } as Suggestion));
      setSuggestions(prev => {
  ...
  const filteredListings = useMemo(() => {
    return listings.filter((l) => {
      ...
      // 2. Asset Class (Hunter Directive) - Added Null Check Safety
      ...
      // 3. Operational Context (Hunter Lifestyle Tags) - Added Null Check Safety
      ...
      if (hunterPreferences?.lifestyle_tags && hunterPreferences.lifestyle_tags.length > 0) {
        ...
        matchesLifestyle = hunterPreferences.lifestyle_tags.some(tag => {
          ...
          if (cleanTag === 'near_hq') return vibes.includes('business') || vibes.includes('central');
          if (cleanTag === 'school_run') return vibes.includes('school') || vibes.includes('family');
          if (cleanTag === 'night_ops') return vibes.includes('nightlife') || vibes.includes('social');
      ...
      return matchesType && matchesGeo && matchesPrice && matchesTrust && matchesAssetClass && matchesLifestyle;
    });
  }, [listings, filterType, drawPolygon, minPrice, maxPrice, showTrustRadar, hunterPreferences]);
  ...
  const onClickMap = (event: any) => {
    ...
    if (!feature) { if (selectedListing) setSelectedListing(null); setShowSuggestions(false); return; }
  ...
  const sidebarVariants = {
    desktop: { x: isSidebarHovered ? 0 : -360, opacity: isSidebarHovered ? 1 : 0.8 },
  ...
  return (
    <div className="relative h-screen w-full bg-asta-deep font-sans overflow-hidden">
      <Map
        ref={mapRef} initialViewState={INITIAL_VIEW_STATE} style={{ width: "100%", height: "100%" }}
        mapStyle={mapStyle} mapboxAccessToken={MAPBOX_TOKEN} interactiveLayerIds={["clusters", "unclustered-point", "price-label"]}
        onClick={onClickMap}
        onContextMenu={(e) => {
          e.preventDefault();
      ...
      >
        {searchBoundary && (
          <Source id="search-boundary" type="geojson" data={searchBoundary}>
            <Layer {...boundaryFillLayer} /><Layer {...boundaryLayer} />
          </Source>
        )}
        ...
        <Source id="listings" type="geojson" data={geojsonData} cluster={true} clusterMaxZoom={14} clusterRadius={50}>
          ...
          <Layer {...unclusteredPointLayer} /><Layer {...priceLabelLayer} />
        </Source>
          ...
          <Marker longitude={draftLocation.long} latitude={draftLocation.lat} draggable={true} onDragEnd={(e) => setDraftLocation({ long: e.lngLat.lng, lat: e.lngLat.lat })} anchor="bottom">
            <div className="flex flex-col items-center gap-1 group cursor-grab active:cursor-grabbing">
              <div className="bg-black/80 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/20 text-white shadow-xl mb-1 flex items-center gap-2">
                ...
                <button onClick={(e) => { e.stopPropagation(); confirmLocation(); }} className="bg-emerald-600 hover:bg-emerald-500 text-white rounded p-1 transition-colors">
        ...
        <div className="absolute top-4 right-4 md:bottom-32 md:top-auto md:right-[10px] flex flex-col gap-2 z-10">
          ...
          <button onClick={handleReset} className="w-[29px] h-[29px] bg-white rounded-md shadow flex items-center justify-center hover:bg-red-50 text-red-600 border border-gray-300 transition-colors" title="Reset Map View"><RotateCcw size={16} /></button>
          <button onClick={() => setMapStyle((s) => s.includes("dark") ? "mapbox://styles/mapbox/streets-v12" : "mapbox://styles/mapbox/dark-v11")} className="w-[29px] h-[29px] bg-white rounded-md shadow flex items-center justify-center hover:bg-gray-100 text-black border border-gray-300" title="Toggle Map Theme">{mapStyle.includes("dark") ? <Sun size={16} /> : <Moon size={16} />}</button>
          <button onClick={() => { setShowHeatmap(!showHeatmap); setShowTrustRadar(false); }} className={`w-[29px] h-[29px] rounded-md shadow flex items-center justify-center transition-all border ${showHeatmap ? "bg-emerald-500 text-white border-emerald-600" : "bg-white text-black border-gray-300 hover:bg-gray-100"}`} title="Toggle Price Heatmap"><Layers size={16} /></button>
          <button onClick={() => { setShowTrustRadar(!showTrustRadar); setShowHeatmap(false); }} className={`w-[29px] h-[29px] rounded-md shadow flex items-center justify-center transition-all border ${showTrustRadar ? "bg-purple-600 text-white border-purple-700" : "bg-white text-black border-gray-300 hover:bg-gray-100"}`} title="Toggle Trust Radar (Verified Assets Only)"><Shield size={16} /></button>
          <button onClick={() => setShowFeedback(true)} className="w-[29px] h-[29px] bg-white rounded-md shadow flex items-center justify-center hover:bg-orange-50 text-orange-500 border border-gray-300 transition-colors" title="Report Issue / Feedback"><Bug size={16} /></button>
          <button onClick={handleManualListing} className="w-[29px] h-[29px] bg-emerald-600 rounded-md shadow flex items-center justify-center hover:bg-emerald-50 text-white border border-emerald-500 transition-colors" title="List New Property"><Plus size={16} /></button>
        </div>

        <motion.div onHoverStart={() => setIsSidebarHovered(true)} onHoverEnd={() => setIsSidebarHovered(false)} animate={window.innerWidth < 768 ? sidebarVariants.mobile : sidebarVariants.desktop} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="absolute z-20 flex pointer-events-auto shadow-2xl md:left-0 md:top-0 md:bottom-0 md:w-[400px] md:h-full md:border-r inset-x-0 bottom-0 h-[80vh] rounded-t-xl border-t border-white/20 md:rounded-none md:border-t-0">
          <div className="flex-1 flex flex-col bg-asta-deep/95 backdrop-blur-md h-full relative">
            ...
            <div className="p-4 border-b border-white/10">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h1 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-emerald-500 animate-pulse"></span>ASTA <span className="text-asta-platinum font-light">LIVE</span></h1>
                <div className="ml-auto">
                  {user ? (
                    <button onClick={() => openDossier('dashboard')} className="text-[10px] font-bold text-emerald-400 border border-emerald-500/30 px-3 py-1.5 rounded hover:bg-emerald-500/10 transition-all flex items-center gap-1.5"><LayoutDashboard size={12} /> DOSSIER</button>
                  ) : (
                    <button onClick={() => setAuthModalOpen(true)} className="text-[10px] font-bold text-emerald-400 border border-emerald-500/30 px-3 py-1.5 rounded hover:bg-emerald-500/10 transition-all flex items-center gap-1.5"><LogIn size={10} /> LOG IN</button>
                  )}
                </div>
              </div>
              <div className="relative mb-3 z-50">
                <div className="relative">
                  <input type="text" placeholder="Search location, feature..." value={searchQuery} onKeyDown={(e) => e.key === "Enter" && executeSearch(searchQuery)} onChange={handleInput} onFocus={() => suggestions.length > 0 && setShowSuggestions(true)} className="w-full bg-black/40 border border-white/10 rounded-lg py-2 pl-3 pr-8 text-sm text-white focus:outline-none focus:border-emerald-500/50 transition-all font-mono" />
                  {isSearching || autocompleteLoading ? <Loader2 className="absolute right-3 top-2.5 text-emerald-500 w-4 h-4 animate-spin" /> : <Search className="absolute right-3 top-2.5 text-gray-500 w-4 h-4" />}
                </div>
                {showSuggestions && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-black/90 border border-white/10 rounded-lg shadow-xl overflow-hidden backdrop-blur-md">
                    {suggestions.map((item, i) => (
                      <div key={i} onClick={() => selectSuggestion(item)} className="flex items-center gap-3 px-3 py-2.5 hover:bg-white/10 cursor-pointer transition-colors border-b border-white/5 last:border-0">
              ...
              <div className={`${!isMobileExpanded && window.innerWidth < 768 ? "hidden" : "block"}`}>
                <div className="flex flex-wrap gap-2 mb-3 max-h-24 overflow-y-auto scrollbar-hide">
                  {trendingTags.map((tag, i) => <button key={i} onClick={() => executeSearch(tag)} className="px-2 py-1 bg-white/5 border border-white/10 rounded text-[10px] text-asta-platinum hover:bg-emerald-500/20 transition-all">#{tag}</button>)}
                </div>
                <div className="flex gap-1 p-1 bg-white/5 rounded-lg mb-3">
                  {["all", "sale", "rent"].map((type) => <button key={type} onClick={() => setFilterType(type as any)} className={`flex-1 py-1.5 text-[10px] uppercase font-bold tracking-wider rounded-md transition-all ${filterType === type ? "bg-emerald-500 text-black" : "text-gray-400 hover:bg-white/5"}`}>{type}</button>)}
                </div>
                <div className="mb-3">
                  <div className="flex justify-between items-center mb-1.5"><span className="text-[10px] text-gray-400 uppercase tracking-wider font-bold">Price Range (GHS)</span></div>
                  <div className="flex items-center gap-2 mb-2 bg-white/5 p-1 rounded-lg border border-white/10">
                    <div className="relative flex-1"><span className="absolute left-2 top-1.5 text-xs text-gray-500">₵</span><input type="number" placeholder="Min" value={minPrice} onChange={(e) => setMinPrice(e.target.value)} className="w-full bg-transparent text-white text-xs pl-5 py-1 focus:outline-none placeholder-gray-600" /></div>
                    <span className="text-gray-600 text-xs">-</span>
                    <div className="relative flex-1"><span className="absolute left-2 top-1.5 text-xs text-gray-500">₵</span><input type="number" placeholder="Max" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} className="w-full bg-transparent text-white text-xs pl-5 py-1 focus:outline-none placeholder-gray-600" /></div>
                  </div>
                  <div className="flex gap-1.5">{["Budget", "Family", "Luxury"].map((label) => <button key={label} onClick={() => applyPricePreset(label)} className="flex-1 py-1 text-[9px] border border-white/10 rounded hover:bg-white/10 text-gray-400 transition-colors uppercase tracking-wide">{label}</button>)}</div>
                </div>
                <button onClick={handleDashboardReset} className="w-full mt-2 flex items-center justify-center gap-2 py-2 rounded border border-red-500/30 text-red-400 hover:bg-red-500/10 hover:text-red-300 hover:border-red-500/50 transition-all text-[10px] uppercase font-bold tracking-widest"><X size={12} /> Clear Dashboard Filters</button>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-4 scrollbar-hide"><AnimatePresence>{filteredListings.map((property) => <ListingCard key={property.id} property={property as any} isSelected={selectedListing?.id === property.id} onClick={() => setSelectedListing(property)} />)}</AnimatePresence></div>
          </div>
          <div className="hidden md:flex w-10 h-full items-center justify-center cursor-pointer group"><div className="w-1 h-12 bg-white/20 rounded-full group-hover:bg-emerald-500 transition-all" /></div>
        </motion.div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#1A1A1A] border border-white/10 w-full max-w-md rounded-xl shadow-2xl overflow-hidden">
                <div className="p-4 border-b border-white/10 flex justify-between items-center bg-red-500/5">
                  <h3 className="text-white font-bold flex items-center gap-2"><Bug className="text-red-500 w-4 h-4" /> Bug Hunter AI</h3>
                  <button onClick={() => setShowFeedback(false)} className="text-gray-400 hover:text-white transition-colors"><X size={18} /></button>
                </div>
                <div className="p-4 space-y-4">
                  <p className="text-[10px] text-gray-500 font-mono uppercase">// describe the system anomaly:</p>
                  <div className="flex gap-2">
                    {[{ id: "location", label: "Map Loc" }, { id: "bug", label: "Software" }, { id: "feature", label: "Request" }].map((type) => (
                      <button key={type.id} onClick={() => setFeedbackType(type.id as any)} className={`flex-1 py-2 text-[10px] font-bold uppercase tracking-wider rounded border transition-all ${feedbackType === type.id ? "bg-red-500 text-white border-red-500" : "bg-white/5 text-gray-400 border-white/10 hover:bg-white/10"}`}>{type.label}</button>
                    ))}
                  </div>
                  <textarea value={feedbackText} onChange={(e) => setFeedbackText(e.target.value)} placeholder="Scanning system for logs..." className="w-full h-32 bg-black border border-white/10 rounded-lg p-3 text-sm text-white placeholder-gray-700 focus:outline-none focus:border-red-500/50 resize-none font-mono" />
                  <button onClick={submitFeedback} disabled={isSubmittingFeedback || !feedbackText} className="w-full bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white font-bold py-3 rounded-lg flex items-center justify-center gap-2 transition-colors uppercase tracking-widest text-xs">
                    {isSubmittingFeedback ? <><Loader2 size={14} className="animate-spin" /> Analyzing...</> : <><Send size={14} /> Transmit Error Log</>}
                  </button>
                </div>
              </motion.div>
            </div>
        ...
        <AnimatePresence>{selectedListing && <PropertyInspector property={selectedListing} onClose={() => { setSelectedListing(null); setSearchParams({}); }} onVerify={() => setVerifyingProp({ id: selectedListing.id.toString(), title: selectedListing.title, })} />}</AnimatePresence>
        <AnimatePresence>{isProfileOpen && <UnifiedCommandCenter onClose={closeDossier} initialSection={dossierSection} />}</AnimatePresence>
        <AnimatePresence>
          {showEmptyState && (
            <motion.div initial={{ y: -100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -100, opacity: 0 }} className="absolute top-8 left-1/2 -translate-x-1/2 z-40 bg-black/90 text-white px-6 py-4 rounded-lg shadow-2xl border border-white/20 flex flex-col items-center gap-2 backdrop-blur-md max-w-sm text-center">
              ...
              <div className="flex gap-2 mt-2 w-full">
                <button onClick={() => { setShowEmptyState(null); if (user) openDossier('hunter'); else setAuthModalOpen(true); }} className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-bold py-2 rounded transition-colors">{showEmptyState.type === "hunter" ? "Activate Hunter Alert" : "Join Waitlist"}</button>
                <button onClick={() => setShowEmptyState(null)} className="px-3 bg-white/10 hover:bg-white/20 text-white text-[10px] font-bold py-2 rounded transition-colors">Dismiss</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>

        <AnimatePresence>{verifyingProp && <FieldReportModal propertyId={verifyingProp.id} propertyTitle={verifyingProp.title} onClose={() => setVerifyingProp(null)} onSuccess={() => {}} />}</AnimatePresence>
        <AnimatePresence>{submitLocation && <SubmitIntelModal location={submitLocation} onClose={() => setSubmitLocation(null)} onSuccess={() => { alert("Intelligence received. Asset is now active on the grid."); }} />}</AnimatePresence>
